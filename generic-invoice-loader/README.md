...
Loads long invoice CSVs (15,000+ rows, any client's format) from a Google Drive folder into BigQuery automatically, every time a new file appears. No computer needs to stay on — this runs as a serverless Cloud Function on a schedule.

Only `.csv` files (and native Google Sheets, which Drive exports as CSV automatically) are picked up.

This uses the same proven pattern as the UPS billing pipeline elsewhere in this repo, but instead of a fixed 64-column schema for one invoice format, this version auto-detects whatever columns are in each file's header row — so it works across different clients' invoice layouts.
...
If every invoice going through this really is the same UPS "CODED" billing format as the `Google Cloud Databse for Climb Accounting` pipeline elsewhere in this repo, use that one instead — it has a proper typed/partitioned schema and reconciliation views (period totals, charge mix) built for that exact format. Use this generic loader when invoices come from different clients or vendors with different column layouts.
